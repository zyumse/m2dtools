...
def angle_distribution(coors, box, cutoff):
    ...
    -------
    numpy.ndarray
        Array of O–O–O angles in degrees.
    """
...
def compute_autocorrelation(forces, max_lag):
    ...
    forces : numpy.ndarray
        Array of shape ``(n_frames, n_atoms, 3)`` containing per-atom forces.
    max_lag : int
    ...
    -------
    numpy.ndarray
        Force autocorrelation values of length ``max_lag``.
    """
...
def compute_msd_pbc(positions, box_lengths, lag_array):
    ...
    box_lengths : numpy.ndarray or list
        Per-frame box lengths of shape ``(n_frames, 3)``.
    lag_array : numpy.ndarray or list
        Lag times for which to compute the MSD.

    Returns
    -------
    numpy.ndarray
        Mean-squared displacement values corresponding to ``lag_array``.
    """
...
def calc_compressibility(V, T=300):
    """Calculate compressibility from volume fluctuations.

    Parameters
    ----------
    V : numpy.ndarray
        Volume history in ``nm^3``.
    T : float, default 300
    ...
    -------
    float
        Isothermal compressibility in ``1/Pa`` (scaled to ``1/GPa``).
    """
    kB = 1.380649e-23  # J/K
